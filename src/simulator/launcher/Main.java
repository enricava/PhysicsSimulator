 ...
 * 
 *  -h
 *  -m gui -i resources/examples/ex4.4body.txt -s 100
 *  -m gui -i resources/examples/ex4.4body.txt -o resources/examples/ex4.4body.out -s 100
 *  -m gui -i resources/examples/ex4.4body.txt -o resources/examples/ex4.4body.out -s 100 -gl ftcg
 *  -m gui -i resources/examples/ex4.4body.txt -o resources/examples/ex4.4body.out -s 100 -gl nlug
 *
 */
 ...
 * Naming conventions:
 * 
 * 	Variables starting with underscore _ are default variables
 */
...
public class Main {
	...
	private static void init() {
		...
		gravityBuilders.add(new NewtonUniversalGravitationBuilder());
	...
	private static Options buildOptions() {
		...
		// output file
		cmdLineOptions
				.addOption(Option.builder("o").longOpt("output").hasArg().desc("Describe JSON output file.").build());
		...
		cmdLineOptions.addOption(Option.builder("s").longOpt("steps").hasArg().desc(
				"An integer representing the number of simulation steps. Default value:" + _nStepsDefaultValue + ".")
				.build());
				...
				.addOption(
						Option.builder("m").longOpt("mode").hasArg()
								.desc("Execution Mode. Possible values: �batch�\n"
										+ "(Batch mode), �gui� (Graphical User Interface). Default value: �batch�.")
								.build());
	...
	private static void parseModeOption(CommandLine line) {
		...
		if (!_exMode.equals("gui") && !_exMode.equals("batch"))
			_exMode = "batch";
	}
	...
	private static void parseInFileOption(CommandLine line) throws ParseException {
		...
		if (_inFile == null && _exMode.equals("batch")) {
	...
	private static void parseOutFileOption(CommandLine line) throws ParseException {
		...
		if (_outFile == null && _exMode.equals("batch")) {
			throw new ParseException("An output file description is required");
		}
	}
	...
	private static void parseNumberStepsOption(CommandLine line) throws ParseException {
		String nsteps = line.getOptionValue("steps", _nStepsDefaultValue.toString());
		try {
			_nSteps = Integer.parseInt(nsteps);
			assert (_nSteps > 0);
		} catch (Exception e) {
			...
			public void run() {
				...
				if(_inFile != null) {
					try {
						...
						System.out.println("Archivo propuesto incorrecto");
					}
